...
export default function EditForm({ onClose, onFormReset, shouldResetForm, selectedRowData }: any) {
  ...
  const submitHandler = async (data: any, { resetForm }: any) => {
    const isCorporate = data.entity === 1;
    const isPersonal = data.entity === 0;
    await dispatch(
      editLpWhitelist({
        ...
        entity: data.entity,
        corporateName: isCorporate ? data.corporateName : null,
        corporateAddress: isCorporate ? data.corporateAddress : null,
        description: data.description,
        name: isPersonal ? data.name : null,
        surname: isPersonal ? data.surname : null,
        birthDate: isPersonal ? data.birthdate : null,
  ...
  useEffect(() => {
    const fetchApis = async () => {
      const { lpName, currency } = selectedRowData;
      const lpNameId = senderLpEnum?.find((item: any) => item.label === lpName)?.value;
      if (lpNameId) {
        const currencyList = await dispatch(postLPTransactionCurrencies({ lpNameId: lpNameId }));
        if (currencyList?.payload?.success) await dispatch(postLPTransactionNetworks({ lpNameId: lpNameId, currencyName: currency }));
      }
      await dispatch(getLpOkxVaspList());
    };
  ...
  return (
    <Formik
      ...
      {({ values, setFieldValue, errors, handleSubmit }) => (
        <Form name='edit-lp-whitelisted' layout='vertical' onFinish={handleSubmit}>
          <CCard paddingOn>
            <Row align={'top'} gutter={36}>
                ...
                <Form.Item name='lpName' label={t('lpName')}>
                  <>
                    <Select
                      ...
                      value={values?.lpName}
                      onChange={(value) => handleFieldChange('lpName', value, setFieldValue, values)}
                      placeholder={t('lpName')}
                      disabled={true}>
              ...
              <Col span={12}>
                <Form.Item name='network' label={t('network')}>
                  <>
                    <Select showSearch optionFilterProp='label' value={values?.network} onChange={(value) => setFieldValue('network', value)} placeholder={t('network')} disabled={true}>
                      {networksList &&
                  ...
                  {values?.lpName === 3 && values?.channel === 0 ? (
                    <>
                      <Select
                        ...
                        defaultValue={values?.vaspName}
                        onChange={(value) => setFieldValue('vaspName', value)}
              ...
              <Col span={12}>
                <Form.Item name='district' label={t('district')}>
                  <>
                    <Input value={values.district} onChange={(event) => setFieldValue('district', event.target.value)} type='text' placeholder={t('district')} disabled={values?.lpName !== 3} />
              ...
              <Col span={12}>
                <Form.Item name='streetName' label={t('streetName')}>
                  <>
                    <Input value={values.streetName} onChange={(event) => setFieldValue('streetName', event.target.value)} type='text' placeholder={t('streetName')} disabled={values?.lpName !== 3} />
